...
import { BookOpen, Lock, Unlock, ChevronRight, Heart, Award, Clock } from "lucide-react";
      ...
      <DashboardShell>
        <div className="max-w-7xl mx-auto space-y-8 pb-20">
          <div className="text-center">
            ...
            <h1 className="text-4xl md:text-5xl font-black tracking-tighter">Stay Alive with Jesus</h1>
            <p className="text-gray-500 mt-2">15 essential lessons for new believers to grow in faith</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {stayAliveLessons.map((lesson) => {
              ...
              return (
                <div key={lesson.id} className="bg-white rounded-2xl border p-5">
                  <div className="flex justify-between items-start mb-3">
                    <span className="text-[10px] font-black text-teal-600 bg-teal-50 px-2 py-1 rounded-full">Lesson {lesson.id}</span>
                    {isUnlocked ? <Unlock size={14} className="text-green-500" /> : <Lock size={14} className="text-gray-400" />}
                  </div>
                  <h3 className="font-black text-lg mb-2">{lesson.title}</h3>
                  ...
                  <button
                    ...
                    className={"w-full py-2.5 rounded-xl font-black text-[10px] uppercase tracking-wider transition " + (isUnlocked ? "bg-teal-600 text-white hover:bg-teal-700" : "bg-gray-100 text-gray-500 hover:bg-gray-200")}
                  >
